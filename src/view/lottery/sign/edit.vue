        ...
        <Row>
            ...
            <Form ref="signForm" :label-width="150">
                <Card :bordered="false" dis-hover>
                    ...
                    <FormItem label="距离误差" style="width:60%;">
                        <Input v-model="detail.distance"></Input>
                        <p>允许用户打卡时所处地理位置与打卡点地理位置最大距离</p>
                    </FormItem>
...
export default {
    ...
    methods: {
        init() {
            let _this = this;
            Utils.ajax.post('api.php?entry=sys&c=lottery-sign&a=edit&do=display', {
                    token: _this.token,
        ...
        saveConfig() {
            let _this = this;
            _this.$Spin.show();
            Utils.ajax.post('api.php?entry=sys&c=lottery-sign&a=edit&do=post', {
                    token: _this.token,
                    lottery_id: _this.lottery_id,
                    id: _this.id,
                    sign: _this.detail
